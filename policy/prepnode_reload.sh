...
do
   echo "allow $IP;" >> /etc/nginx/conf.d/prepnode_dynamic_ips.conf
done

oldcksum=`cksum /etc/nginx/conf.d/prepnode_dynamic_ips.conf`

inotifywait -e modify,move,create,delete -mr --timefmt '%d/%m/%y %H:%M' --format '%T' /etc/nginx/conf.d/ | while read date time; do

newcksum=`cksum /etc/nginx/conf.d/prepnode_dynamic_ips.conf`
