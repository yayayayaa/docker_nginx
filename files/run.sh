#!/bin/bash
export USER_NGINX_ALLOWIP=${USER_NGINX_ALLOWIP:-"no"} # node IP 외 allow ip 추가 여부 (yes/no)
export PREP_MODE=${PREP_MODE:-"no"} # nginx allow ip 가 dynamic 할 경우 (yes/no)
export PREP_NODE_LIST_API=${PREP_NODE_LIST_API:-""} # node ip 들 확인하기 위한 URL
